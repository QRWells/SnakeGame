...
public class LoginMessageHandler extends
    SimpleChannelReadHandler<LoginMessage> {
  ...
  @Override
  public void read0(MessageHandlerContext ctx, LoginMessage msg)
      throws Throwable {
    //database
    ...
    if (user == null) {
      ...
      responseDetail = user.getId()
                           .toString();
      ServerContext.INSTANCE.getUsers()
                            .put(user.getId()
                                     .getValue(), ctx.channel());
      ServerContext.INSTANCE.getUserGroup()
                            .get(user.getId()
                                     .getValue())
                            .forEach(groupId -> {
                              if (!ServerContext.INSTANCE.getGroups()
                                                         .containsKey(
                                                             groupId)) {
                                ServerContext.INSTANCE.getGroups()
                                                      .put(groupId,
                                                           new HashSet<>());
                              }
                              ServerContext.INSTANCE.getGroups()
    ...
    ctx.channel()
       .write(new ResponseMessage(response, responseDetail));
    ctx.channel()
