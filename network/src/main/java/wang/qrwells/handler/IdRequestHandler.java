...
public class IdRequestHandler extends SimpleChannelReadHandler<IdRequestMessage> {
  ...
  public void read0(MessageHandlerContext ctx, IdRequestMessage msg) throws Throwable {
    if (msg.isGroup()) {
      ...
      if (user == null) {
        ctx.channel().write(new ResponseMessage(ResponseMessage.Status.ERROR, "user not found"));
      } else {
        log.info("Found user: " + user.getName());
        ctx.channel().write(new ResponseMessage(ResponseMessage.Status.OK, user.getName()));
      }
    }
    ctx.channel().writeAndFlush(new TimestampMessage());
  }
}
